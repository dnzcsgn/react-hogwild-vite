...
describe("Hog App", () => {
  ...
  it("displays additional hog details when a tile is clicked", () => {
    ...
    expect(
      screen.getByText(`Specialty: ${hogs[index].specialty}`),
    ).toBeInTheDocument();
    expect(screen.getByText(hogs[index].weight)).toBeInTheDocument();
    expect(
      screen.getByText(hogs[index].greased ? "Greased" : "Nongreased"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(hogs[index]["highest medal achieved"]),
    ).toBeInTheDocument();
  });
  ...
  it("sorts hogs by name or weight", () => {
    ...
    const renderedHogNamesByName = screen
    ...
    const renderedHogNamesByWeight = screen
      .getAllByRole("heading", { level: 3 })
      .map((el) => el.textContent);

    expect(renderedHogNamesByWeight).toEqual(
      sortedByWeight.map((hog) => hog.name),
    );
  });
  ...
  it("hides a hog when the hide button is clicked", () => {
    ...
    const sortedHogsByName = [...hogs].sort((a, b) =>
      a.name.localeCompare(b.name),
    );

    expect(
      screen.queryByText(sortedHogsByName[0].name),
